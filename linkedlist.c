...
void list_reverse(list_t *list) {
	...
	listnode_t *L = list->head, *R = list->tail;
	void *tmp;
	while (L != R) {
		tmp = L->elem;
		L->elem = R->elem;
...
// Special list functions for Simon
void list_moveiter(list_iter_t *iter) {
	if (iter->node == NULL) fatal_error("list iterator exhausted");
	else iter->node = iter->node->next;
}
void *list_getcurrent(list_iter_t *iter) {
	if (iter->node == NULL) fatal_error("node doesn't exist");
	return iter->node->elem;
}
void list_removecurrent(list_iter_t *iter) {
	if (iter->node == NULL) fatal_error("node to be removed doesn't exist");
